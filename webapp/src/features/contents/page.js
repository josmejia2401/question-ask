...
import { filterItems, createItem, editItemById, deleteItemById } from './api';
class Page extends React.Component {
    ...
    handleEdit = async (item) => {
        try {
            const response = await editItemById(item.id, item);
            const indexData = this.state.data.findIndex(value => value.id === item.id);
            if (indexData !== -1) {
                this.state.data[indexData] = item;
            }
            const indexDataFiltered = this.state.dataFiltered.findIndex(value => value.id === item.id);
            if (indexDataFiltered !== -1) {
                this.state.dataFiltered[indexDataFiltered] = item;
            }
            this.updateState({
                data: this.state.data,
                dataFiltered: this.state.dataFiltered,
    ...
    handleDelete = async (item) => {
        try {
            const response = await deleteItemById(item.id);
            const indexData = this.state.data.findIndex(value => value.id === item.id);
            if (indexData !== -1) {
                this.state.data.splice(indexData, 1);
            }
            const indexDataFiltered = this.state.dataFiltered.findIndex(value => value.id === item.id);
            if (indexDataFiltered !== -1) {
                this.state.dataFiltered.splice(indexDataFiltered, 1);
            }
            this.updateState({
                data: this.state.data,
                dataFiltered: this.state.dataFiltered,
    ...
    renderModalContent = () => {
        ...
        switch (modalType) {
            case 'edit':
                return <EditItem item={selectedItem}
                    onSubmit={this.handleEdit}
                    onClose={this.closeModal} />;
            case 'delete':
                return <DeleteItem item={selectedItem}
                            ...
                            <div className="card-header">
                                <div style={{ flexDirection: "column", display: 'flex', width: '100%' }}>
                                    <div style={{ flexDirection: "row", display: 'flex', width: '100%' }}>
                                        <h4 className="card-title title-color">Listado de contenido</h4>
                                ...
                                <div className="table-responsive">
                                    <table className="table table-hover mb-0">
                                        <thead>
                                            ...
                                            <tr>
                                                <th><b>Título</b><i className="fa fa-fw fa-sort" onClick={() => this.handleSortTableByColumn('title')}></i></th>
                                                <th><b>Descripción</b><i className="fa fa-fw fa-sort" onClick={() => this.handleSortTableByColumn('description')}></i></th>
                                                <th><b>Estado</b><i className="fa fa-fw fa-sort" onClick={() => this.handleSortTableByColumn('statusId')}></i></th>
                                                <th><b>Fecha creación</b><i className="fa fa-fw fa-sort" onClick={() => this.handleSortTableByColumn('createAt')}></i></th>
                                                <th>Acción</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            {this.state.loading && (<tr>
                                                <td className="text-color align-items-center text-center" colSpan={5}>
                                            ...
                                            {!this.state.loading && this.state.dataFiltered.length > 0 && this.state.dataFiltered.map((item, index) => {
                                                return (<tr key={index}>
                                                    <td>
                                                        <div className="row" style={{ width: '100%', height: '60px' }}>
                                                            <div className="d-flex align-items-center" style={{ alignItems: 'center' }}>
                                                                <span className={`text-color`}>{truncateText(item.title, 50)}</span>
                                                            </div>
                                                        </div>
                                                    </td>
                                                    <td>
                                                        <div className="row" style={{ width: '100%', height: '60px' }}>
                                                            <div className="d-flex align-items-center" style={{ alignItems: 'center' }}>
                                                                <span className={`text-color`}>{truncateText(item.description, 50)}</span>
                                                            </div>
                                                        </div>
                                                    </td>
                                                    <td>
                                                    ...
                                                    <td>
                                                        <div className="" style={{ minWidth: '70px', height: '60px' }}>
                                                            <div className="d-flex align-content-center flex-wrap" style={{ alignItems: 'center' }}>
                                                                <Link to={"#"} style={{ padding: '2px' }}>
                                                                    <i className="fa-regular fa-pen-to-square" onClick={() => this.openModal('edit', item)}></i>
                                                                </Link>
                                                                <Link to={"#"} style={{ padding: '2px' }}>
                                                                    <i className="fa-solid fa-trash" onClick={() => this.openModal('delete', item)}></i>
                                                                </Link>
                                                            </div>
                                                        </div>
                                                    </td>
                                        ...
                                        {this.state.lastEvaluatedKey && !this.state.loading && !this.state.downloading && this.state.dataFiltered.length > 0 && (<tfoot>
                                            <tr>
                                                <td colSpan={5} style={{ textAlign: 'center', alignContent: 'center', alignItems: 'center', alignSelf: 'center' }}>
